...
@Component({
  selector: 'create-modify-warehouse-timeline-modal',
  templateUrl: './create-modify-warehouse-timeline-modal.component.html',
  styleUrls: ['./create-modify-warehouse-timeline-modal.component.less']
})
export class CreateModifyWarehouseTimelineModalComponent implements OnChanges {
  @Input() modifyingModel: WarehouseTimelineModel = null;
  @Input() visible: boolean = false;
  @Output() handleVisible = new EventEmitter<boolean>();
  @Output() afterSubmit = new EventEmitter();
  @ViewChild('createModifyForm') createModifyForm: NgForm;
  isProcessing: boolean = false;
  model: WarehouseTimelineModel = new WarehouseTimelineModel({ status: Status.NEW });
  statusList = [Status.NEW, Status.ACTIVE];
  showExplainNumberOfDay: boolean = false;
  ...
  init() {
    if (this.modifyingModel) {
      ...
      this.model = new WarehouseTimelineModel({ status: Status.NEW });
    }
  }
  ...
  async submit() {
    ...
    if (this.createModifyForm.valid && !_.isEmpty(this.model.name)) {
      ...
      let response;
      if (this.modifyingModel) {
        response = await this.warehouseService.updateTimeline(this.model);
      } else {
        response = await this.warehouseService.createTimeline(this.model);
      }

...
export class AddRegionModalComponent implements OnChanges {
    ...
    @Input() regionModel: FlatLocationModel = new FlatLocationModel();
    @Input() visible: boolean = false;
    @Output() handleVisible = new EventEmitter();
    @Output() afterSubmit = new EventEmitter();
    @ViewChild('addRegionForm') addRegionForm: NgForm;
    ...
    ngOnChanges() {
        if (this.visible) {
            this.model = new FlatLocationModel(this.regionModel);
        }
    }
    ...
    async submit() {
        ...
        if (this.addRegionForm.valid) {
            ...
            let response;
            this.model.level = LocationLevel.REGION;
            if (this.regionModel) {
            ...
            if (response.errorCode === 0) {
                ...
                this.messageService.success(`${this.translateService.instant(`actions.${this.regionModel ? 'update' : 'add'}`)} ${this.translateService.instant('common.successfully').toLowerCase()}`);
                this.reset();
            } else {
                this.messageService.error(response.message);
            }
        } else {
            CommonHelper.validateForm(this.addRegionForm);
        }
    }
    ...
    reset() {
        this.init();
        CommonHelper.resetForm(this.addRegionForm);
    }
